#!/usr/bin/env python3
"""
Spring Training - SparkSubmitOperator ile Profesyonel Çözüm
...
dag = DAG(
    'spring_training_spark_submit_pro',
    default_args=default_args,
    description='🌸 Spring Training - SparkSubmitOperator (Professional)',
...
# 2. 🚀 SparkSubmitOperator - PROFESYONEL ÇÖZÜM (DÜZELTİLMİŞ)
spring_model_training = SparkSubmitOperator(
    task_id='spark_submit_spring_training',
    
    # 📍 Spark Connection
    conn_id='spark_default',
    
    # 📁 Application Path
    application='/workspace/pipelines/4-silver_layer/seasonal_models/spring_training.py',
    ...
    # 🔧 Spark Configuration (Master URL burada belirtiliyor)
    conf={
    ...
    # 📝 Naming
    name='Spring_Season_Model_Training_{{ ds }}',
...
check_training_results = BashOperator(
    ...
    bash_command="""
    echo "📊 Model eğitimi sonuçları kontrol ediliyor..."
    
    # Spark application loglarını kontrol et
    if docker exec development-spark-client bash -c "ls -la /workspace/logs/spring_training_*.log 2>/dev/null"; then
        echo "✅ Log dosyaları bulundu"
        
        # Son log dosyasındaki başarı mesajını ara
        LATEST_LOG=$(docker exec development-spark-client bash -c "ls -t /workspace/logs/spring_training_*.log 2>/dev/null | head -1")
        ...
        if [ ! -z "$LATEST_LOG" ]; then
            ...
            if docker exec development-spark-client grep -q "Model eğitimi tamamlandı" "$LATEST_LOG"; then
                ...
                echo "⚠️  Model eğitimi durumu belirsiz"
            fi
        fi
    else
...
cleanup_spark_history = BashOperator(
    ...
    bash_command="""
    echo "🧹 Eski Spark application'ları temizleniyor..."
    ...
    docker exec development-spark-client bash -c "
        find /workspace/logs -name 'spring_training_*.log' -mtime +7 -delete 2>/dev/null || true
        echo '✅ 7+ günlük log dosyları temizlendi'
    "
    
    echo "✅ Spark geçmişi temizleme tamamlandı"
    """,
...
check_spark_health >> spring_model_training >> check_training_results >> cleanup_spark_history
