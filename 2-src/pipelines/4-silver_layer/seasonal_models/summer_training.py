...
# ✅ SPRING GİBİ BASIT MLflow IMPORT
import mlflow
import mlflow.spark
...
def check_data_availability(target_year=2016):
    ...
    print("🔍 30 Eylül tarihine kadar veri kontrolü...")
    ...
    try:
        ...
        if max_date_str >= target_end_date:
            print("✅ 30 Eylül tarihine kadar veri mevcut!")
            return True
        else:
            print(f"⏳ Henüz 30 Eylül verisi gelmedi. Son: {max_date_str}")
            return False
...
def extract_summer_data(target_year=2016):
    """Yaz sezonu verisi çek"""
    print(f"📊 {target_year} yılı yaz verisi çekiliyor...")
    
    try:
        # Yaz sezonu: Haziran-Eylül (6-9. aylar)
        query = f"""
        SELECT 
        ...
        AND EXTRACT(MONTH FROM full_timestamp) BETWEEN 6 AND 9
        ...
        if count == 0:
            print("❌ Yaz sezonu verisi bulunamadı!")
            return None
...
def train_model(feature_df, target_col, model_name="summer_energy_model", target_year=2016):
    """Model eğitimi"""
    ...
    try:
        ...
        # Random Forest modeli - Yaz için biraz daha karmaşık
...
def main():
    ...
    model_name = "summer_energy_model"
    
    print("☀️ Yaz sezonu model eğitimi başlıyor...")
    ...
    try:
        ...
        # 2. Veri çekme
        daily_df = extract_summer_data(target_year)
        if daily_df is None:
            sys.exit(1)
        ...
        if success:
            print("☀️ Yaz sezonu model eğitimi tamamlandı!")
            print("📍 Model eğitimi Spark Client'te başarıyla tamamlandı")
        else:
            print("❌ Yaz sezonu model eğitimi başarısız!")
            sys.exit(1)
